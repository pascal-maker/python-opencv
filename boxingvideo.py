...
def check_foul(defender, shooter):
    ...
    for dpart in def_parts:
        for spart in shoot_parts:
            if calculate_distance(defender[dpart], shooter[spart]) < 40:  # arbitrary threshold, may need tuning
                return True
    return False
...
while cap.isOpened():
    ...
    if success:
        ...
        if len(results) >= 2:  # only check if we have at least two people detected
            ...
            shooter, defender = humans[:2]  # assuming first person is shooter and second is defender
            ...
            if check_foul(persons[1], persons[0]):
                ...
                blue_frame = cv2.addWeighted(
                    annotated_frame, 0.7, np.zeros(frame.shape, frame.dtype) + [0, 0, 255], 0.3, 0
                )
